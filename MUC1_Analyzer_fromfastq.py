...
def write_merged_pdf(path, sample, frag, score, haps, rel_dupc=None, rs_detected=None, rs_alert=None):
    ...
    # ── SECTION 2 — MUC1_Score (SINGLE AXIS: frameshift tail + cohort position + phenotype flag) ──────
    ...
    if rs_detected and rs_detected.get("base"):
        rs_src = f"read off the mutant allele — C:{rs_detected['C']} / T:{rs_detected['T']}, depth {rs_detected['depth']}"
    elif rs_detected and rs_detected.get("label") == "low_depth":
        rs_src = f"insufficient depth (C:{rs_detected['C']} / T:{rs_detected['T']})"
    else:
        rs_src = score.get("splice_source") or "provided"
    _rs_base = score.get("splice_base")
    rs_txt = (f"{_rs_base} on the mutant allele — for information only, NOT used in the score ({rs_src})"
              if _rs_base else "not determined — for information only, NOT used in the score")
    _tail = score.get("fs_tail")
    _pct = score.get("fs_percentile")
    tail_txt = (f"{_tail:g} repeats downstream of the variant" if _tail is not None else "n/a")
    place_txt = (f"percentile {_pct * 100:.0f}%  (z {score.get('fs_z'):+.2f}; "
                 f"cohort μ={score.get('fs_mu'):g} σ={score.get('fs_sd'):g}, n={score.get('fs_n')})"
                 if _pct is not None else "n/a")
    rows = [["MUC1_Score — frameshift tail", tail_txt],
            ["Position on the cohort curve", place_txt],
            ["rs4072037 (mutant allele)", rs_txt],
            ["VNTR ratio (mut/healthy)", f"{score.get('ratio')}" if score.get("ratio") is not None else "n/a"]]
    ...
    # cohort calibration curve + red sample dot (frameshift tail on the Gaussian)
    if _tail is not None:
        try:
            from muc1_analyzer.report_blocks import fs_tail_plot
            _p = fs_tail_plot(_tail)
            if _p is not None:
                story += [_p, Spacer(1, 0.25 * cm)]
        except Exception as e:
            story.append(Paragraph(f"<i>cohort curve unavailable ({e})</i>", META))

    # genotype → phenotype flag, driven by the frameshift-tail category (4 categories)
    _cat = score.get("fs_category")
    _label = (score.get("fs_label") or "").upper()
    _color = score.get("fs_color") or "#888"
    if _cat and _label:
        story.append(Paragraph(f'<font color="{_color}"><b>{_label}</b></font>', GENO))
    else:
        story.append(Paragraph('<font color="#888"><b>MUC1 SCORE UNAVAILABLE — no measured frameshift tail</b></font>', GENO))
    if score.get("fs_confident") is False:
        story.append(Spacer(1, 0.1 * cm))
        story.append(Paragraph('<font color="#B8860B"><b>⚠ category provisional — frameshift position '
                               'not reliably measured</b></font>', META))
        ...
        if not contigs:
            ...
            if per_allele_haps or clair3_frag:
                try:
                    ...
                    summary["muc1_score"] = score                       # single-axis (frameshift tail)
                    summary["muc1_two_axis_score"] = score              # legacy key alias (back-compat)
                    ...
                    print(f"[cigar-len] MUC1_Score: {clinical_string(frag, per_allele_haps)}  carrier={frag.get('mutation_present')} "
                          f"frameshift_tail={score.get('fs_tail')} → {score.get('fs_category')} "
                          f"(rs4072037 {score.get('splice_base')}, info only)", file=sys.stderr)
                    # ribbon reconstruction figure (DEFAULT output) — the motif-array picture of both
                    # haplotypes, from the merged json; verdict from the dupc layer so the variant is
                    # highlighted only on a POSITIVE call. Never allowed to sink the run.
                    try:
                        from muc1_analyzer import vntr_ribbon
                        _mj = summary.get("merged_haplotypes_json")
                        _rv = rel_dupc.get("verdict") if isinstance(rel_dupc, dict) else None
                        if _mj:
                            _rib = os.path.join(a.outdir, f"{a.sample}.ribbon.png")
                            if vntr_ribbon.render_safe(_mj, _rib, verdict=_rv):
                                summary["ribbon_png"] = _rib
                                print(f"[cigar-len] ribbon figure → {_rib}", file=sys.stderr)
                    except Exception as _e:
                        print(f"[cigar-len] ribbon figure skipped ({_e})", file=sys.stderr)
                except Exception as e:
