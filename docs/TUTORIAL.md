...
From a runnable synthetic quickstart (no patient data) to the real-data workflow and the two-axis
...
# (c) two-axis MUC1_Score from the analyzer output
python -m muc1_analyzer score --analyzer-json /tmp/demo/DEMO.json --snp-genotype 1
```

The synthetic reads are pure motif A (no frameshift), so no mutation is called and the onset axis is
...
# 4) two-axis MUC1_Score (rs4072037 from your genotyping: --snp-genotype 0/1/2 or --rs4072037-mut C/T)
...
this is the *genotype*; the severity axis wants the base on the **mutant** allele (see §6 below).
...
#   -> onset_index (from position/length), severity (rs4072037 splice), ratio; provenance tagged
```

The two axes are reported separately: **onset** (frameshift position) and **severity** (mutant-allele
rs4072037 splice, C = severe / T = protected). Weights are direction-of-effect, uncalibrated.
...
## 6. Phasing rs4072037 with the mutant allele (severity axis)

The severity axis is the splice haplotype **of the mutant allele** — not the genotype. A homozygote is
...
Precedence for the severity axis, highest first:
...
A length-homozygous heterozygote has no cis handle: the severity axis is left **unset** rather than
...
| Two-axis score (auto) | `python -m muc1_analyzer score --analyzer-json S.json --snp-genotype 1` |
| Two-axis score (visual) | `python -m muc1_analyzer score --clinical-call "…(variant @ repeat N)" --rs4072037-mut C` |
