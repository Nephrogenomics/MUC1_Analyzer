...
def _tail_bb(n: int, k: int, a: float, b: float) -> float:
    """P(X >= k) for X ~ BetaBinom(n, a, b) — the OVERDISPERSED counterpart of `_tail`.

    A binomial assumes every position shares one background rate and that all remaining variance is
    sampling. Measured per-index profiles violate that: the background wanders between positions
    (max/median 1.2-1.6 on real reads, 2.0 on simulated), so a binomial charges that extra spread to the
    alternative and calls it a variant. The Beta prior on `p` (mean a/(a+b), overdispersion
    rho = 1/(a+b+1)) absorbs it: at rho -> 0 this converges back to `_tail`.

    Same log-space summation as `_tail` (lgamma + logsumexp) for the same overflow reason.
    """
    if k <= 0:
        return 1.0
    if k > n:
        return 0.0
    if a <= 0.0 or b <= 0.0:
        return _tail(n, k, a / (a + b) if (a + b) > 0 else 0.0)
    lg_n1 = lgamma(n + 1)
    lbeta_ab = lgamma(a) + lgamma(b) - lgamma(a + b)
    mode = (n + 1) * a / (a + b)             # peak of the beta-binomial pmf
    terms = []
    cur_max = float("-inf")
    for i in range(k, n + 1):
        lt = (lg_n1 - lgamma(i + 1) - lgamma(n - i + 1)
              + lgamma(i + a) + lgamma(n - i + b) - lgamma(n + a + b) - lbeta_ab)
        terms.append(lt)
        if lt > cur_max:
            cur_max = lt
        if i > mode and lt < cur_max - 45.0:  # upper tail exhausted -> stop
