...
# KNOWN ADTKD-MUC1 variants (the canonical dupC : insertion of a C -> frameshift).
# A frameshift outside this list stays pathogenic (candidate) but 'known_pathogenic=False'.
...
def from_analyzer_json(path: str) -> dict:
    ...
    carriers = [p for p in parsed if p["has_mut"]]
    ...
    if mut and mut["variants"]:
        # representative : prefer known, then exact
        v = sorted(mut["variants"], key=lambda x: (not x["known_pathogenic"], not x["exact"]))[0]
