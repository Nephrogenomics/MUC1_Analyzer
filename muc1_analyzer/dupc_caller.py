...
def parse_f_null(value):
    """CLI/env string → the `f_null` argument of `call_dupc`. Pure (testable).

    ⚠ It exists because the two entry points had DRIFTED APART: `call_dupc`/`run` default to `adaptive`
    since 2026-07-29, while this module's CLI still defaulted to the literal `0.02` and accepted only
    `auto` or a float — so `--f-null adaptive` raised a ValueError, and any sweep driven through the CLI
    (`run_spec.sbatch`) silently measured the OLD null while reporting on the new default. A gate that
    tests a different caller than the one shipped is worse than no gate."""
    s = str(value).strip().lower()
    if s in ("adaptive", "auto"):
        return s
    try:
        return float(s)
    except ValueError:
        raise ValueError(f"--f-null: expected 'adaptive', 'auto' or a float, got {value!r}")
...
def call_dupc(bam: str, *, hp_mut=None, pon=None, ctx_range=(3, 4, 5), genome_ref: str = None,
              ...
              probe: str = "ctract", f_null="adaptive", any_len: bool = False) -> dict:
    ...
    `f_null` (positional path): float = the historical fixed background (0.02, the ONT R10 assumption),
    or `"auto"` = estimate background AND overdispersion from the sample's own per-position profile
    (beta-binomial). Use `"auto"` on any read set whose chemistry/basecaller is not ours — simulated
    reads, another platform, another lab. Measured 2026-07-28: a simulated WT with a 13.6 % background is
    called at p_bonf 1e-104 against a fixed 0.02, purely by arithmetic. Default stays fixed so validated
    clinical behaviour is byte-identical.

    `any_len` (positional path): test EVERY anomalous C-tract length rather than only `mut_len` — finds
    58_59insG (9C) or a 2 bp deletion (5C) without being told what to expect, with Bonferroni over the
    lengths tested. `mut_len` in the result then reports the winning length.
    """
    if probe == "unitlen":
    ...
    if mut_len in (8, 5) and out.get("tier") != "called":
        try:
            ...
            if use_seq:
                ...
                pos = scan_dupc_positional_seq(bam, region=region, genome_ref=genome_ref,
                                               mut_len=mut_len, wt_len=wt_len, f_null=f_null,
                                               any_len=any_len)
            else:
                from .detectors.vntr_dupc import scan_dupc_positional
                pos = scan_dupc_positional(bam, genome_ref=genome_ref, mut_len=mut_len, f_null=f_null)
            out["positional"] = {"called": pos.get("called"), "best": pos.get("best"),
                                 "n_span": pos.get("n_span"), "anchor": pos.get("anchor", "genomic"),
                                 "frame": pos.get("frame"), "frames": pos.get("frames"),
                                 "f_null_used": pos.get("f_null_used"),
                                 "f_null_mode": pos.get("f_null_mode"),
                                 "f_null_observed": pos.get("f_null_observed"),
                                 "rho": pos.get("rho"), "noise_flags": pos.get("noise_flags"),
                                 **({"mut_len_called": pos.get("mut_len"),
                                     "cand_lens": pos.get("cand_lens")} if any_len else {})}
            # surface the noise diagnosis at top level: a flagged sample must not need a JSON dive
            if pos.get("noise_flags"):
                out["noise_flags"] = pos["noise_flags"]
            if pos.get("called"):
                out["tier"] = "positional"
                out["called"] = True
...
if __name__ == "__main__":
    ...
    ap.add_argument("--f-null", dest="f_null", default="adaptive",
                    help="positional null: 'adaptive' (DEFAULT — the validated constant 0.02, switching "
                         "to in-sample estimation only when the sample's own background trips "
                         "high_background), 'auto' = always estimate background AND overdispersion from "
                         "the sample itself (beta-binomial), or a float. Use 'auto'/'adaptive' for "
                         "foreign chemistry/basecaller or SIMULATED reads: against a fixed 0.02, a set "
                         "whose true floor is 0.14 is called on arithmetic alone")
    ap.add_argument("--any-len", action="store_true",
                    help="positional: test EVERY anomalous C-tract length, not only --mut-len (finds "
                         "58_59insG=9C, 2bp deletions=5C without being told), Bonferroni over lengths")
    args = ap.parse_args()
    f_null = parse_f_null(args.f_null)
    ...
    res = call_dupc(args.bam, hp_mut=args.hp_mut, pon=pon,
                    ...
                    probe=args.probe, f_null=f_null, any_len=args.any_len)
