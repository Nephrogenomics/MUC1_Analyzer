...
def build_summary(genomic_bam, haps, *, chrom="chr1", snp_pos=None, copy_offset=4,
                  ...
                  gate_min_reads=5, gate_min_strand=2, smear=False):
    ...
    decoded on it is an artefact (the GHO false 60dupA). A SUPPLIED mutation is authoritative, never gated.
    ...
    # Composite MUC1_Score (two-axis), folded in HERE so the report/PDF is the SINGLE deliverable (no separate
    # score.json to reconcile). onset_index = 1 − repeat/mut_len (Spearman −0.89 vs age-of-onset); severity =
    # ± from the MUTANT allele's rs4072037 (C = severe +, T = protected −). composite = onset + severity scores.
    prognosis = None
    if onset and onset.get("repeat") is not None and onset.get("mutant_length"):
        ...
        comp = (round(sc["onset_score"] + sc["severity_score"], 3)
                if sc.get("onset_score") is not None and sc.get("severity_score") is not None else None)
        prognosis = {"onset_index": sc.get("onset_index"), "onset_score": sc.get("onset_score"),
                     "severity_score": sc.get("severity_score"), "composite": comp,
...
def render_text(s):
    ...
    L = ["", "  " + "─" * 76,
         "  TWO-AXIS PROGNOSIS  (from the genomic BAM)",
         "  " + "─" * 76]
    ...
    if s.get("onset"):
        ...
        L.append(f"  → ONSET signal    : {o['variant']}{rep} "
    ...
    if s.get("severity"):
        v = s["severity"]
        tag = "OBSERVED" if v["observed"] else "from genotype"
        if v.get("low_conf"):
            tag += ", TENTATIVE — mutant allele length low-confidence"
        L.append(f"  → SEVERITY signal : mutant allele carries rs4072037-{v['rs4072037']} "
                 f"= {v['interpretation']}  [{tag}]")
    if not s.get("onset"):
        L.append("  → no frameshift on either allele (non-carrier / caller-invisible variant)")
    if s.get("prognosis"):
        p = s["prognosis"]
        comp = f"{p['composite']:+.2f}" if p.get("composite") is not None else "n/a (severity unresolved)"
        oi = p.get("onset_index")
        gate = "  [⚠ coverage-gated — onset unreliable]" if p.get("coverage_gated") else ""
        L.append(f"  → MUC1_Score (two-axis): composite {comp}  (onset_index {oi}, "
                 f"onset {p.get('onset_score')} + severity {p.get('severity_score')}){gate}")
    L.append("  " + "─" * 76)
    return "\n".join(L) + "\n"
...
def render_pdf_blocks(s, styles):
    ...
    out.append(Paragraph("Two-axis prognosis (genomic BAM)", styles["result"]))
    rows = []
    if s["arbiter"].get("long_low_conf") or not s["arbiter"].get("alleles"):
    ...
    if s.get("onset"):
        ...
        rows.append(f"<b>ONSET:</b> {o['variant']} ~ repeat {o['repeat']} "
                    f"(mutant allele {o['mutant_length']} copies){gated}")
    if s.get("severity"):
        v = s["severity"]
        rows.append(f"<b>SEVERITY:</b> mutant allele carries rs4072037-{v['rs4072037']} "
                    f"= {v['interpretation']}")
    if s.get("prognosis"):
        p = s["prognosis"]
        comp = f"{p['composite']:+.2f}" if p.get("composite") is not None else "n/a (severity unresolved)"
        gate = ("  <font color='#b00'>[⚠ coverage-gated]</font>" if p.get("coverage_gated") else "")
        rows.append(f"<b>MUC1_Score (two-axis):</b> composite {comp} "
                    f"(onset_index {p.get('onset_index')}){gate}")
    for r in rows:
