...
So the PCR mode:
  ...
  6. reports a FOUR-way status — positive / negative / **undetermined** / low_coverage — so a sample whose
     internal null is corrupted (one carrier: the WT/control allele's own 8C rate is itself at calling level, from a
     strand-asymmetric homopolymer artifact) surfaces as UNDETERMINED, not a silent False. That carrier's *mutant*
...
def call_from_reads(reads, *, mut_ctract: int = 8, floor: float = 0.10, min_abs: float = 0.07,
    ...
    as a negative (each carries `flags` for transparency):
      ...
      * "undetermined"  — no call, but the internal NULL is corrupted: the WT/control (lower) allele's own 8C
                          ...
                          null → we cannot trust a negative. This is that carrier (WT allele 0.085 rev-only; its mutant
...
def _decide(per, *, floor: float = 0.10, min_abs: float = 0.07, min_ratio: float = 1.4) -> dict:
    ...
    if len(measurable) == 2:                                   # internal null (mutant vs the other allele)
        ...
        # separates that carrier (WT allele 0.085, rev-only, mutant only 1.27× above it → report UNDETERMINED) from
        ...
        for name, p in per.items():
            if p["frac"] is not None and p["frac"] >= floor and p["strand_ok"]:
                ...
                break
    # Status: a call → positive; else a corrupted internal null (untrustworthy negative) → undetermined;
