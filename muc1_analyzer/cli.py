...
def parse_args(argv=None):
    p = argparse.ArgumentParser(
        prog="muc1_score",
        description="MUC1_Score — single-axis ADTKD-MUC1 score (frameshift tail length) from a patient "
                    "CRAM/BAM (Phase 1) or from fragments (Phase 0). See docs/ROADMAP.")
    ...
    p.add_argument("--snp-genotype", type=int, choices=[0, 1, 2], default=None,
                   help="rs4072037 dosage (0/1/2). Reported FOR INFORMATION only — rs4072037 has no "
                        "independent prognostic value and is NOT used in the MUC1_Score.")
    ...
    # ── SINGLE-AXIS score (public) : the frameshift TAIL length (mut_len − position) on the cohort curve ──
    p.add_argument("--clinical-call", dest="clinical_call", default=None,
                   help="[SCORE] visually/clinically finalized call, e.g. "
                        "'64 repeats | 80 repeats (del8_27 @ repeat 37)' → single-axis MUC1_Score "
                        "(frameshift tail). For variants finalized by eye (del8_27) or coverage-floored het long-VNTR.")
    p.add_argument("--rs4072037-mut", dest="rs4072037_mut", choices=["C", "T"], default=None,
                   help="observed rs4072037 base of the MUTANT allele (visual review / dRNA-seq); reported "
                        "FOR INFORMATION only (rs4072037 is not part of the score), else --snp-genotype + cis rule.")
    p.add_argument("--w-onset", type=float, default=2.0,
                   help="[DEPRECATED — no effect; onset abandoned] kept for CLI compatibility")
    p.add_argument("--w-splice", type=float, default=1.0,
                   help="[DEPRECATED — no effect; rs4072037 informational] kept for CLI compatibility")
    p.add_argument("--legacy-composite", action="store_true",
                   help="[dev] use the OLD additive ratio composite (score.py) instead of the single-axis "
                        "frameshift-tail score. For reproducibility only.")
    return p.parse_args(argv)


def _run_clinical_call(args) -> int:
    """SINGLE-AXIS MUC1_Score from a visual/clinical call string (frameshift tail; rs4072037 informational)."""
    ...
    res["score_model"] = "single-axis (frameshift tail = mut_len − position ; rs4072037 informational)"
    ...
    if args.output == "-":
        ...
        with open(args.output, "w") as fh:
            fh.write(out + "\n")
        print(f"[OK] MUC1_Score (frameshift tail) → {args.output}", file=sys.stderr)
    if res.get("carrier"):
        print(f"[MUC1_Score single-axis] {args.sample or '?'}: "
              f"frameshift tail={res.get('fs_tail')} rep → {res.get('fs_category')} "
              f"(percentile {res.get('fs_percentile')}, z {res.get('fs_z')}) · "
              f"rs4072037 splice={res.get('splice_base')} (info only) · "
              f"mut={res.get('mut_len')}/healthy={res.get('healthy_len')} "
              f"ratio={res.get('ratio')}", file=sys.stderr)
    else:
        print(f"[MUC1_Score single-axis] {args.sample or '?'}: non-carrier "
...
def _run_phase1(args) -> int:
    ...
    row.update({k: ta[k] for k in ("mut_is_long", "ratio", "onset_index", "onset_score",
                                   ...
                                   "fs_tail", "fs_z", "fs_percentile", "fs_category", "fs_label",
                                   "fs_color", "fs_mu", "fs_sd", "fs_n", "fs_confident",
                                   "rs4072037_informational", "score_axis")})
    row["score_model"] = "single-axis (frameshift tail)"
    ...
    print(f"[MUC1_Score single-axis] {row.get('sample')}: frameshift tail={row.get('fs_tail')} rep → "
          f"{row.get('fs_category')} (percentile {row.get('fs_percentile')}, z {row.get('fs_z')}) · "
          f"rs4072037={row.get('splice_base')} (info only) · "
...
def main(argv=None):
    ...
    if args.analyzer_json and not args.legacy_composite:
        ...
        res = score_from_fields(frag.get("vntr_len_mut"), frag.get("vntr_len_healthy"), pos,
                                ...
                                w_onset=args.w_onset, w_splice=args.w_splice)
        # `mutation_source` travels WITH the verdict: since the consensus can no longer veto the dupC
        # caller (2026-07-29), a carrier=True whose only witness is the caller is exactly the class of
        # verdict that changed — auditable from the score JSON alone, without a pre-fix baseline run.
        # ⚠ `variant` read `mv.get("name")`, and `mutation_variant` has NO `name` key — it carries `motif`
        # (consensus branch) or `label` (dupC-caller branch). So every score.json ever written on this
        # route said `variant: null`, including for a confirmed carrier, while the analyzer JSON two
        # levels down named the variant correctly. Found 2026-09-04 reading a control panel, where a row
        # of `carrier=True variant=None` looked like a caller that fires without identifying anything.
        res.update(sample=args.sample, carrier=frag.get("mutation_present"),
                   variant=(mv.get("motif") or mv.get("label")),
                   variant_repeat=mv.get("repeat_index"), variant_exact=mv.get("exact"),
                   mutation_source=frag.get("mutation_source"),
                   dupc_caller_called=frag.get("dupc_caller_called"),
                   score_model="single-axis (frameshift tail = mut_len − position ; rs4072037 informational)")
        out = json.dumps(res, indent=2, ensure_ascii=False)
        (print(out) if args.output == "-" else open(args.output, "w").write(out + "\n"))
        print(f"[MUC1_Score single-axis] {args.sample or '?'}: frameshift tail={res.get('fs_tail')} rep → "
              f"{res.get('fs_category')} (percentile {res.get('fs_percentile')}) · "
              f"rs4072037 splice={res.get('splice_base')} (info only) · "
